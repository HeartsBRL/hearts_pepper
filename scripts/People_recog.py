...
class LiftTask(PepperController):
    ...
    def on_human_tracked(self, value):
        ...
        """
        print "face_detected_VARIABLE is " + str(self.face_detected)
        if value == []:  # empty value when the face disappears
            self.got_face = False
            self.face_detected = False
        elif not self.got_face:  # only speak the first time a face appears
            # print "got_face_VARIABLE is " + str(self.got_face)
            self.got_face = True
            # print "got_face_VARIABLE is " + str(self.got_face)
            print "I saw a face!"
    ...
    def on_word_tracked(self, value):
        ...
        """
        word = value[0]
        print "phrase is: " + str(word)
        print "confidence is: " + str(value[1])
        if value == []:  # empty value when the face disappears
            self.got_word = False
            self.word_detected = False
        elif not self.got_word:  # only speak the first time a face appears
            # print "got_face_VARIABLE is " + str(self.got_face)
            self.got_word = True
            # print "got_face_VARIABLE is " + str(self.got_face)
    ...
    def runwordDetect(self):
        """
        Loop on, wait for events until manual interruption.
        """
        print "Starting WordDetection"
        # liftTask.say("\\vol=30\\Detecting faces mode activated")
    ...
    def runfaceDetect(self):
        ...
        Loop on, wait for events until manual interruption.
        """
        print "Starting HumanDetection"
        # liftTask.say("\\vol=30\\Detecting faces mode activated")
        while self.face_detected == False:
            time.sleep(0.1)
        # self.ttsProxy.say("Hello, you!")
        print "Found target"
        # liftTask.untrack("Sound")
    ...
    def runfaceGone(self):
        """
        Loop on, wait for events until manual interruption.
        """
        print "Starting HumanDetection"
        # liftTask.say("\\vol=30\\Disappeared faces mode activated")
        while self.face_detected == True:
            time.sleep(0.1)
        # self.ttsProxy.say("Where did you go?")
        print "Lost target"
        self.face_detection.unsubscribe("HumanGreeter")
        # liftTask.untrack("Face")
        liftTask.untrack()
...
####People face tracking#####
    ...
    liftTask.Subscribe2Speech()
    liftTask.runwordDetect()
    ...
    # liftTask.face_detected = False
# try:
#     while True:
#         liftTask.trackSound()
#         print liftTask.trackerProxy.getRegisteredTargets()
#         # liftTask.say("\\vol=30\\Tracking sounds mode activated")
#         liftTask.faceDetect()
#         liftTask.runfaceDetect()
#         print liftTask.trackerProxy.getRegisteredTargets()
#         liftTask.startRecogPeople()
#         print liftTask.trackerProxy.getRegisteredTargets()
#         # liftTask.say("\\vol=30\\Tracking humans faces mode activated")
#         liftTask.faceDetect()
#         liftTask.runfaceGone()
#         print liftTask.trackerProxy.getRegisteredTargets()
# except KeyboardInterrupt:
#     print "Interrupted by user"
#     print "Stopping..."
